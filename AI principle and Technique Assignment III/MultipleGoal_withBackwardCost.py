...
def uniform_cost_search_multiple_goals_with_paths(start, goals):
    ...
    while priority_queue:
        ...
        # If we reach a goal state, record it
        if current_city in goals and current_city not in found_goals:
            found_goals[current_city] = (path, current_cost)
            print(f"Reached goal: {current_city} with total cost: {current_cost} via path: {' -> '.join(path)}")
...
# Example usage
goals = ["Axum", "Gondar", "Lalibela", "Babile", "Jimma", "Bale", "Sofu Oumer", "Arba Minch"]
found_paths = uniform_cost_search_multiple_goals_with_paths("Addis Ababa", goals)
